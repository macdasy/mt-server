...
def create_pie_chart(data, total_income, total_outcome, labels):
    ...
    if len(data) > 1:
        ...
        # labels = sorted(labels, key=lambda x: x[1], reverse=True)[:5]
        labels.append(['Others', total - sum(data)])
...
def create(data, govt_list, table_headings, chart_data, total_income, 
            total_outcome, line_chart_data, current_lang, outflow_labels, 
            inflow_labels, running_balance, running_balance_item, gross_income, gross_outcome ):
    ...
    for d in data:
        ...
        for row in d[1:]:
            table_data.append([
                Paragraph( "{:,}".format(float(cell)), table_cell_Style) if isAmount(str(cell)) else Paragraph(str(cell), table_cell_Style)
                for cell in row
                ])
    ...
    for d in govt_list:
        ...
        col_width = []
        if (table_headings[heading_index] == "Interest credited and debited") or (table_headings[heading_index] == "DEDUCTION"):
    ...
    closure_table = [
        ...
        ['Transactions', '{:.2f}'.format(total_outcome[1]), '{:.2f}'.format(total_income[1])],
    ]
    ...
    table_data.append(
        [Paragraph( "{:,}".format(float(cell)), table_cell_Style) if isAmount(str(cell)) else Paragraph(str(cell), table_cell_Style) 
         for cell in closure_table[1]])

    table_data.append(
        [Paragraph( "{:,}".format(float(cell)), table_cell_Style) if isAmount(str(cell)) else Paragraph(str(cell), table_cell_Style)
         for cell in closure_table[2]])
    ...
    heading_text = "Running balance check"
    heading = Paragraph(heading_text, getSampleStyleSheet()['Heading1'])
    elements.append(heading)
    

    if type(running_balance) == bool:
        if running_balance:
            heading_text = "VERIFIED AND CORRECT"
            heading = Paragraph(heading_text, header_style)
            elements.append(heading)
        
        else:
            heading_text = "VERIFIED AND INCORRECT"
            heading = Paragraph(heading_text, header_style)
            elements.append(heading)
    
    else:
        heading_text = "Running Balance is not available!"
        heading = Paragraph(heading_text, header_style)
        elements.append(heading)
